...
class System(pl.LightningModule):
    ...
    def forward(self, *args, **kwargs):
        ...
        results = self.model_enhc(*args, **kwargs)
        results_norm = normalize_estimates_cascaded_tensor(results, args[0])
        # new_args = tuple([results_norm[:, 1, :]])  # taking only mixture and discarding the noise - for gammatone
        new_args = tuple([results_norm[:, 0, :]])  # taking only mixture and discarding the noise - for free
        if False:
            test_dir = r'C:\Users\eliashivk\Documents\DSP_final_project\asteroid-master\egs\librimix\ConvTasNet\exp\train_gammatone_cascaded\Tests'
            batch_ind = 0
            sig = results_norm.squeeze(0).cpu().data.numpy()
            sf.write(
                r'C:\Users\eliashivk\Documents\DSP_final_project\asteroid-master\egs\librimix\ConvTasNet\exp\train_free_cascaded\test\after_enhc_sep_noisy_batch{}.wav'.format(batch_ind),
                sig[batch_ind, 0, :],
                8000,
            )
            mix = args[0].squeeze(0).cpu().data.numpy()
            sf.write(
                r'C:\Users\eliashivk\Documents\DSP_final_project\asteroid-master\egs\librimix\ConvTasNet\exp\train_free_cascaded\test\before_enhc_sep_noisy_batch{}.wav'.format(batch_ind),
                mix[batch_ind, :],
                8000,
            )
            result_sep = self.model_sep(*new_args, **kwargs)
            result_sep_np = result_sep.squeeze(0).cpu().data.numpy()
            results_sep_norm = normalize_estimates(result_sep_np[batch_ind,:,:], new_args[0][batch_ind].squeeze(0).cpu().data.numpy())
            s1 = results_sep_norm[0, :]
            s2 = results_sep_norm[1, :]
            sf.write(
                r'C:\Users\eliashivk\Documents\DSP_final_project\asteroid-master\egs\librimix\ConvTasNet\exp\train_free_cascaded\test\after_sep_s1_batch{}.wav'.format(batch_ind),
                s1,
                8000,
            )
            sf.write(
                r'C:\Users\eliashivk\Documents\DSP_final_project\asteroid-master\egs\librimix\ConvTasNet\exp\train_free_cascaded\test\after_sep_s2__batch{}.wav'.format(batch_ind),
                s2,
                8000,
            )

        return self.model_sep(*new_args, **kwargs)
